...
@admin_required
def admin_dashboard():
    total_users = g.db.fetch_val("SELECT COUNT(*) FROM users")
    total_reviews = g.db.fetch_val("SELECT COUNT(*) FROM videos")
    total_likes = g.db.fetch_val("SELECT COUNT(*) FROM likes")
    total_comments = g.db.fetch_val("SELECT COUNT(*) FROM comments")
...
def admin_reviews_list():
    ...
    where_clauses = ["(v.what LIKE ?1 OR v.where LIKE ?1 OR u.email LIKE ?1)"]
    params = [search_term]

    if status != "all":
        where_clauses.append(f"v.status = ?{len(params) + 1}")
        params.append(status)
    
    where_sql = " AND ".join(where_clauses)
    
    count_query = f"SELECT COUNT(*) FROM videos v JOIN users u ON v.user_id = u.id WHERE {where_sql}"
    total_items = g.db.fetch_val(count_query, tuple(params))
    ...
    select_query = f"""
        ...
        WHERE {where_sql} ORDER BY v.id DESC LIMIT ?{len(params)-1} OFFSET ?{len(params)}
    """
...
def _handle_review_status_change(video_id: int, new_status: str):
    g.db.execute("UPDATE videos SET status = ?1 WHERE id = ?2", (new_status, video_id))
...
def admin_users_list():
    ...
    count_query = "SELECT COUNT(*) FROM users WHERE email LIKE ?1"
    total_items = g.db.fetch_val(count_query, (search_term,))
    total_pages = math.ceil(total_items / settings.ITEMS_PER_PAGE) if total_items > 0 else 0
    
    select_query = "SELECT * FROM users WHERE email LIKE ?1 ORDER BY id LIMIT ?2 OFFSET ?3"
...
def admin_delete_user(user_id: int):
    ...
    g.db.execute("DELETE FROM users WHERE id = ?1", (user_id,))
...
def resolve_complaint(complaint_id: int):
    lang = session.get('lang', 'en')
    g.db.execute("UPDATE complaints SET status = 'resolved' WHERE id = ?1", (complaint_id,))
