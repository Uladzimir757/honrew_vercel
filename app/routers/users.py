...
# --- Остальные функции (обновление профиля, пароля, удаление) остаются без изменений ---
...
def handle_avatar_upload():
    ...
    try:
        ...
        s3_client.put_object(Bucket=settings.S3_BUCKET_NAME, Key=unique_filename, Body=file_content, ContentType=avatar_file.content_type)
    ...
    if g.user["avatar_filename"]:
        try:
            s3_client.delete_object(Bucket=settings.S3_BUCKET_NAME, Key=g.user["avatar_filename"])
        except Exception as e:
...
def request_profile_deletion():
    ...
    g.db.execute("UPDATE users SET delete_token = %s, delete_token_expires = %s WHERE id = %s", (token, expires.isoformat(), g.user["id"]))
    delete_link = url_for('users.confirm_profile_deletion', token=token, lang=lang, _external=True)
    send_email_notification(recipients=[g.user["email"]], subject_key="email_delete_account_subject", body_key="email_delete_account_body", template_vars={"delete_link": delete_link})
...
def confirm_profile_deletion(token: str):
    # ... код без изменений, так как CASCADE в БД все сделает сам
    lang = session.get('lang', 'en')
    now_utc_iso = datetime.now(timezone.utc).isoformat()
    user_to_delete = g.db.fetch_one("SELECT * FROM users WHERE delete_token = %s AND delete_token_expires > %s", (token, now_utc_iso))
    ...
    if user_to_delete["avatar_filename"]:
        try:
            s3_client = boto3.client('s3', endpoint_url=settings.S3_ENDPOINT_URL, aws_access_key_id=settings.AWS_ACCESS_KEY_ID, aws_secret_access_key=settings.AWS_SECRET_ACCESS_KEY, region_name="auto")
            s3_client.delete_object(Bucket=settings.S3_BUCKET_NAME, Key=user_to_delete["avatar_filename"])
        except Exception as e:
            logger.error(f"Ошибка при удалении аватара пользователя {user_id_to_delete} из R2: {e}")
